...
//checking email if already exists
   $ret=mysqli_query($con, "call sp_userpwdrecoveryvalidation('$username','$email')");
...
echo "<script>alert('username or Email id is invalid');</script>";
         ...
         $ret->close();
         $con->next_result();
     $ret=mysqli_query($con,"call sp_userpasswordrecovery('$username','$email','$password','$updatetTime')");
    if ($ret) {
